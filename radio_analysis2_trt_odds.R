...
dat <- trt_yn |> 
  ...
  radio_dat_clean() |>
  radio_exclusions_comparisons() |>
  arrange(term, ctry_order)
...
radio_analysis2_table |> saveRDS("results/radio_analysis2_table.RDS")
...
p <- p + meta_diamond(y_shift = 20, groupvar = term)
...
p <- p + theme_bw() +
  ...
  icbp_colour_manual()
p 
ggsave("results/figure2_radio.pdf",
       plot = p,
       width = 15,
       height = 11.25,
       units = "cm")
ggsave("results/figure2_radio.svg",
       plot = p,
       width = 15,
       height = 11.25,
       units = "cm")
