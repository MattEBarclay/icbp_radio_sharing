...
table <- trt_q_perc |>
  ...
  group_by(site_order) |>
  radio_exclusions_all(jurisdiction = j_order, cancer = site_order) |>
...
radio_analysis3_table |> saveRDS("results/radio_analysis3_table.RDS")
...
p <- p + theme_bw() +
  ...
  icbp_colour_manual()
p 
ggsave("results/radio_time1_overall.png",
       plot = p,
       width = 15,
       height = 15,
       units = "cm")
...
p 
ggsave("results/radio_time1_overall_alternative1.png",
       plot = p,
       width = 15,
       height = 13,
       units = "cm")
