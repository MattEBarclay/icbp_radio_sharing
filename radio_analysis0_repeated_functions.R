...
radio_exclusions_all <- function(data, jurisdiction = jurisdiction, cancer = cancer) {
...
radio_exclusions_comparisons <- function(data) {
...
# Radiotherapy very common plot options -----------------------------------
theme_icbp <- function() {
...
site_yscale_use <- function() {
  scale_y_continuous(
    ...
    breaks=1:10, 
    minor_breaks = NULL, 
    labels = c("All 8 cancers", cancer_radio_order),  
    trans="reverse"
  ) 
}
  ...
  geom_errorbarh(
    data = poly_metan |> 
      ...
      mutate(
        ...
        ctry_order = factor(jurisdiction, levels = country_order_reml2)
      ),
    height=.3,
    colour = "darkgrey",
    linetype = "solid",
