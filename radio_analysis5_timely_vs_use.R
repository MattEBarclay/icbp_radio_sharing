...
dat_timely <- trt_q_perc |> 
  ...
  mutate(ctry_order  = factor(jurisdiction, levels = country_order)) |>
...
p <- p + scale_y_continuous(
  name = "Average radiotherapy usage (%)", 
...
p 
ggsave("results/figure5_radio_correlation_appendix.svg",
       plot = p,
       width = 15,
       height = 15,
       units = "cm")
ggsave("results/figure5_radio_correlation_appendix.pdf",
       plot = p,
       width = 15,
       height = 15,
       units = "cm")
