## Meta-analysis - dotplot of radio/radio by country and cancer
...
dat2 <- trt_unadj_n |>
  ...
  mutate(cancer = "Rectal, stages II/III") |>
  select(trt, jurisdiction, ordering, cancer, n, n_trt, prop, lower, upper) 

# apply common cleaning and exclusion rules
site_rad2 <- c(site_rad, "Age 15-64", "Age 65-74", "Age 75-84", "Age 85-99", "Men", "Women")

dat <- bind_rows(dat1, dat2) |>
  mutate(site_order  = factor(cancer, levels = site_rad2)) |>
  radio_dat_clean() |>
  radio_exclusions_all() |>
  arrange(site_order, ordering)

rm(dat1, dat2, site_rad2)

# Appendix table of relevant data ------------------------------------------
appdx_analysis1 <- dat |> 
  select(site_order, jurisdiction, n, n_trt, prop, lower, upper) |>
  group_by(site_order) |>
  filter(!is.na(n)) |>
  gt() |>
  gt_appdx_common() |>
...
radio_analysis1_table |> saveRDS("results/radio_analysis1_table.RDS")



# Main version of meta-analysis plot --------------------------------------
dat <- dat |> filter(cancer %in% site_rad)

# Meta-analysis radiotherapy cancer-site ordering
radio_ordering <- dat |>
  filter(!exclude) |>
  ...
  arrange(prop)
radio_ordering
cancer_radio_order <- radio_ordering$cancer
...
p <- p + meta_diamond(y_shift = 20, groupvar = site_order)
# y axis
p <- p + jdn_yscale()
# x axis
p <- p + prop_xscale()
...
p <- p + theme_bw() +
  ...
  icbp_colour_manual() 
p 
ggsave("results/radio_overall_props.png",
       plot = p,
       width = 15,
       height = 22.5,
       units = "cm")
...
dat <- dat |> mutate(
  site_order = factor(cancer, levels = c("All 8 cancers", cancer_radio_order))
)
...
p 
ggsave("results/radio_overall_props_alternative1.png",
       plot = p,
       width = 15,
       height = 13,
       units = "cm")
...
trial <- trial |>
  right_join(extras) |>
  mutate(site_order = factor(cancer, levels = cancer_radio_order)) |>
...
p 
ggsave("results/radio_overall_props_alternative2_lines.png",
       plot = p,
       width = 15,
       height = 13,
       units = "cm")
